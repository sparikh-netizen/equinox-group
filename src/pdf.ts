...
import { DESIGN, PAGE, PATHS } from "./config.js";
...
export async function generatePrintPdf(
  ...
  const pdf = await PDFDocument.load(await readFile(PATHS.staticTemplatePdf));
  pdf.registerFontkit(fontkit);
  const regular = await pdf.embedFont(await readFile(PATHS.regularFont), { subset: true });
  const extraBold = await pdf.embedFont(await readFile(PATHS.extraBoldFont), { subset: true });
  ...
  const textFit = fields.map(({ field, value, font }) => {
    ...
    return {
      ...
      value,
      width,
      maximumWidth: DESIGN.employee.maximumWidth,
      passed: width <= DESIGN.employee.maximumWidth,
    };
  });
  ...
  return { textFit, qrModules: matrix.modules };
}
